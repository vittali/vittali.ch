
:r11: http://dev.ti.com/tirex/explore/node?devtools=LAUNCHXL-CC1352P1&node=AFU6dryAU8NukjdRPy4T4Q__pTTHBmu__LATEST[Sensor and Collector - TI 15.4-Stack Project Zero]
:r12: http://www.ti.com/tool/LAUNCHXL-CC1352R1[SimpleLink™ Multi-Band CC1352R Wireless MCU LaunchPad™ Development Kit]

== Sensor and Collector - TI 15.4-Stack Project Zero

The collector application is less complicated than the coprocessor. Therefore,
I start my experiments with a sensor-collector network. Before passing to the sensor-coprocessor setup, I make sure to disconnect the collector from the host
so that the sensor is forced to connect to the collector part of the coprocessor.

To familiarize myself with the sensor and collector hardware,
I follow the lab exercise {r1}. I use _GTKTerm_ from the Mint/Ubuntu distribution.

I use two {r12} LaunchPads:

[[hardware, hardware setup]]
.Two CC1352P1 launch pads programmed as Sub-1 Ghz sensor and collector
image::sensor-collector-hardware.png[CC1352P1]


=== Task 0

Since I want to connect the two LaunchPads simultaneously, I need to give each
LaunchPad.footnote:[more precisely, the debug probe included on the LaunchPad]
a unique serial number.

==== Setting the serial number of the collector board

I label the collector board (see <<hardware>>) , verify that it is the only board connected to the host
 and set the serial number to `1`:

[source, bash]
----
cd $CCS_INSTALLDIR/ccs/ccs_base/common/uscif/xds110
$ ./xdsdfu -m  <1>
$ ./xdsdfu -s 1 -r <2>
----
<1> set the device into DFU mode (required)
<2> set the serial number to _1_ and reset the device


==== Setting the serial number of the sensor board

I label the sensor board (see <<hardware>>) , verify that it is the only board connected to the host
 and set the serial number to `2`:

[source, bash]
----
cd $CCS_INSTALLDIR/ccs/ccs_base/common/uscif/xds110
$ ./xdsdfu -m  <1>
$ ./xdsdfu -s 2 -r <2>
----
<1> set the device into DFU mode (required)
<2> set the serial number to _2_ and reset the device

=== Copy the example projects

I import the following projects into the CCS workspace as described in the
SimpleLink CC13x2 26x2 SDK (3.40.00.02) documentation of the {r12}

. sensor_CC1352P1_LAUNCHXL_tirtos_ccs_syscfg
. collector_CC1352P1_LAUNCHXL_tirtos_ccs_syscfg
. coprocessor_CC1352P1_LAUNCHXL_tirtos_ccs_syscfg

Within CCS, I then copy the projects into {r0}, with the following abbreviated names:

. sensor
. collector
. coprocessor


=== Edit the target configuration

Each CCS project needs to be configured such that the programs for collector and sensor are loaded to the correct LaunchPad:

[[targetconf, target configuration]]
.Both target configurations are populated with the choosen serial numbers (shown is the collector project).
image::target-conf.png[target configuration]


=== Task 1

I can't set the Sub-1 Ghz Frequency to 868 Mhz as I expected,
the only choice for the european band seems to be 863 Mhz. This might be related
to the channel selection, see <<e2e1>>.

.Configure the TI 15.4 Stack Sub 1-Ghz Frequency
image::sensor-syscfg.png[sensor.syscfg]


=== Task 3

On my host box, I select `/dev/ttyACM0` for the collector and `dev/ttyACM2` for the
sensor virtual UART connection.

As mentioned in step 6, I press and hold `Button 2` and then the `Reset` button (while Button 2 is held down), see <<hardware>>.

.Collector (top) and sensor (bottom) terminal output after factory reset.
image::terminals.png[terminal output after reset]

Some of the steps 7 to 9 are also different for me:

[start=7]
. The collector starts immediately (without waiting for `Button 1`). The output is:

 TI Collector
 Started
 Channel: 0

. as described
. the terminal output of the sensor is:


.The terminal output differs slightly from the one shown in the project description:

.Collector (top) and sensor (bottom) terminal output after the sensor joined the network.
image::sensor-has-joined.png[sensor has joined]

=== Problems with flashing on the LaunchPad

On one occasion, the collector binary flashed to the LaunchPad produced scrambled output on the terminal connection.
I reflashed, recompiled the program, to no avail. I then used UniFlash to clear the entire flash and this solved the problem:

.Clearing the flash with UniFlash
image::erase-all.png[erase flash]
