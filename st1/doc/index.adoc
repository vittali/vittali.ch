...
You are now ready to start the first debug session. Before doing so, I show how to activate the System Analyzer. This is an optional step and applies to all
TI-RTOS based projects, not only to SensorTag projects.

[[logging]]
== Using the System Analyzer

The `README.md` file of the sample project claims that the System Analyzer Unit is enabled.
This is not correct and if you open the System Analyzer (Tools -> System Analyzer -> Duration in the Debug Perspective), you get this:

.System Analyzer
image::DAC.png[Analysis Configuration]

We first need to enable the System Analyzer by modifying the TI-RTOS config file `uartecho.cfg` <<e2e7>>:
...
If possible, use the {rTI-3} for new applications. This kit has the same footprint, similar sensors and is part of the SimpleLink product family.
It therefore enjoys better documentation, tool support and Linux support for the BLE stack.
